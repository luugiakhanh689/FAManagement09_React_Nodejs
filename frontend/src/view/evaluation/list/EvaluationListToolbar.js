...
class EvaluationToolbar extends Component {
  doExport = () => {
    ...
    return (
      <Toolbar>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Link to="/audit-logs?entityNames=evaluation">
...
function select(state) {
  return {
    ...
    hasPermissionToCreate: evaluationSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: evaluationSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(EvaluationToolbar);
